...
//    drawHeader(header_);
  ...
  TH1* plotDataMC(const BinInfo& var_info, const vector<TString> mc_samples, TString data_sample, const Category& category, bool norm_to_data = false, TString norm_cut = "", bool plotlog = false, std::function<void(TCanvas*)> *plotextra = nullptr){
    ...
    vector<TString> mc = {"ttbar", "wjets","znunu","minor","qcd" };
    for (auto &scomb : mc){
      TH1 *hist = nullptr;
      for (auto &sname : mc_samples){
        const auto& sample = config.samples.at(sname);
        TString sMC = TString(sname);
	// if(sMC.Contains("16"))             sMC = sMC.ReplaceAll("-16","");
	// else if(sMC.Contains("17BtoE")) sMC = sMC.ReplaceAll("-17BtoE","");
	// else if(sMC.Contains("17F"))    sMC = sMC.ReplaceAll("-17F","");
	// else if(sMC.Contains("18preHEM"))  sMC = sMC.ReplaceAll("-18preHEM","");
	// else if(sMC.Contains("18postHEM")) sMC = sMC.ReplaceAll("-18postHEM","");
  ...
  TH1* plotDataMC(const BinInfo& var_info, const vector<TString> mc_samples, const vector<TString> data_sample, const Category& category, bool norm_to_data = false, TString norm_cut = "", bool plotlog = false, std::function<void(TCanvas*)> *plotextra = nullptr){
	...
	TString sMC = TString(sname);
	//if(sMC.Contains("16"))             sMC = sMC.ReplaceAll("-16","");
	    if(sMC.Contains("ttbar"))             sMC ="ttbar";
	    //else if(sMC.Contains("17BtoE")) sMC = sMC.ReplaceAll("-17BtoE","");
	      else if(sMC.Contains("wjets")) sMC = "wjets";
	    // else if(sMC.Contains("17F"))    sMC = sMC.ReplaceAll("-17F","");
	      else if(sMC.Contains("znunu")) sMC = "znunu";
	      else if(sMC.Contains("minor")) sMC = "minor";
	    //else if(sMC.Contains("18preHEM"))  sMC = sMC.ReplaceAll("-18preHEM","");
              else if(sMC.Contains("qcd")) sMC = "qcd";
	    // else if(sMC.Contains("18postHEM")) sMC = sMC.ReplaceAll("-18postHEM","");
	    //  sMC = sMC.ReplaceAll("-cr","");
            //    sMC = sMC.ReplaceAll("-withveto","");
	    if(sMC == scomb){
      ...
      if(hist != nullptr){
      vector<Color_t> colors ={2,3,6,7,8};
      //prepHists({hist,false,true,false,colors});
      prepHists({hist});
      if (saveHists_) saveHist(hist);
      mchists.push_back(hist);
      hist->SetFillColor(hist->GetLineColor()); hist->SetFillStyle(1001); hist->SetLineColor(kBlack);
