...
public:
  ...
  void naiveTF(){
    sumYields({"qcd-withveto-16-cr"},"qcd-withveto-cr");
    sumYields({"qcd-withveto-16-sr"},"qcd-withveto-sr");
  ...
  void calcTF(){
    ...
    cerr << "\n--->" << __func__ << endl;
    vector<TString> intsamp={"qcd-16-sr-int"};   
    doYieldsCalc(intsamp, runBootstrapping ? 50 : 0);
   
    vector<TString> qcdsamp = {"qcd-16-sr"};
    if(splitTF){ 
      for (auto &s : intsamp){
      qcdsamp.push_back(s);
    }
    ...
    sumYields({"qcd-16-cr"},"qcd-cr");
    sumYields({"qcd-16-sr"},"qcd-sr");
    sumYields({"qcd-16-sr-int"},"qcd-sr-int");
    // FIXME
  ...
  void calcDataCorr(){
    ...
    vector<TString> calc_samples = {"ttbar-16-cr","wjets-16-cr","minor-16-cr","znunu-16-cr","data-16-cr"};
    doYieldsCalc(calc_samples);
    doYieldsCalc({"qcd-withveto-16-sr", "qcd-withveto-16-cr"}, runBootstrapping ? 50 : 0);
    sumYields({"ttbar-16-cr"},"ttbar-cr");
    sumYields({"wjets-16-cr"},"wjets-cr");
    sumYields({"znunu-16-cr"},"znunu-cr");
    sumYields({"minor-16-cr"},"minor-cr");
    sumYields({"qcd-withveto-16-cr"},"qcd-withveto-cr");
    sumYields({"qcd-withveto-16-sr"},"qcd-withveto-sr");
    sumYields({"data-16-cr"},"data-cr");
    doYieldsCalc({"data-16-norm"});
    sumYields({"data-16-norm"},"data-norm");
		 doYieldsCalc({"ttbar-16-norm","wjets-16-norm","minor-16-norm","qcd-16-norm"});
    sumYields({"ttbar-16-norm"},"ttbar-norm");
    sumYields({"wjets-16-norm"},"wjets-norm");
    sumYields({"minor-16-norm"},"minor-norm");
    sumYields({"qcd-16-norm"},"qcd-norm");
    vector<TString> otherbkg_samples = {"ttbar-cr", "wjets-cr", "minor-cr",};
    vector<TString> norm_samples = {"ttbar-16-norm","wjets-16-norm","minor-16-norm","qcd-16-norm"};
