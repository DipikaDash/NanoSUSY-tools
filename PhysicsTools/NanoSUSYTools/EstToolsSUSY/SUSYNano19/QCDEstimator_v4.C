...
vector<Quantity> QCDPred(){
  ...
  z.printYieldsTableLatex({"_DATA","QCD_data","_QCDTF", "_TF", "_pred"}, labelMap, "yields_qcd_lm_18D.tex","lm", digits); //LM
  if(z.splitTF){
    z.printYieldsTableLatex({"_DATA","QCD_data","_QCDTF", "_TF", "_QCDTF_CR_to_SR_noextrap", "_QCDTF_SR_extrap", "_pred"}, labelMap, "yields_qcd_hm_18D.tex","hm", digits);
  }else{
    z.printYieldsTableLatex({"_DATA","QCD_data","_QCDTF", "_TF", "_pred"}, labelMap, "yields_qcd_hm_18D.tex","hm", digits);
  }
  ...
  c->Print(z.config.outputdir+"validation_18D.png");
