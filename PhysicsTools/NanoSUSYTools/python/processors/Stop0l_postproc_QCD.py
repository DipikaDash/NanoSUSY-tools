...
def main(args):
    ...
    elif process=="smear": p=PostProcessor(args.outputfile,files,cut="MET_pt > 200", branchsel=None, outputbranchsel="keep_and_drop_smear.txt", outputbranchselsmear="keep_and_drop_smear.txt",typeofprocess="smear",modules=mods,provenance=False)
    elif process=="qcdsf": p=PostProcessor(args.outputfile,files,cut="MET_pt > 200 & nJet >= 2", branchsel=None, outputbranchsel="keep_and_drop_sf.txt", modules=mods,provenance=False,maxEvents=args.maxEvents)
    p.run()
