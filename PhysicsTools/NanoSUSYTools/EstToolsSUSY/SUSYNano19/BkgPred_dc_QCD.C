...
void runBkgPred(){
  ...
  vector<const BaseEstimator*> allPreds = {&q, &s};
  ...
  json j;
  j["binlist"] = binlist;
  j["binNum"] = qcdcrBinNumMap;
  j["unitSRNum"] = qcdcrUnitNumMap;
  j["unitCRNum"] = qcdcrUnitCRNumMap;
  j["binMaps"] = binMaps;
...
void runSignalYields(){
  ...
  for (const auto &signal : signals){
    ...
    QCDEstimator q(qcdcrSignalConfig(signal));
    q.calcYields();
    q.convertYields("qcdcr_"+signal, "qcdcr");
    yieldsMap.insert(q.std_yields.begin(), q.std_yields.end());
  }
