...
const TString outputdir = ".";
  ...
  const TString lumistr = "35.9"; //Units are in pb
//2017
  //  const TString lumistr = "41.817";
  //const TString lumistr = "28.08";
  //const TString lumistr = "13.54";
...
// lumi and base weight
  const TString wgtvar = lumistr+"*1000*Stop0l_evtWeight*puWeight*BTagWeight*PrefireWeight";//*Stop0l_trigger_eff_MET_low_dm_QCD";//*ISRWeight"; //2016 //17FpuWeight
//const TString wgtvar = lumistr+"*1000*Stop0l_evtWeight*Stop0l_trigger_eff_MET_loose_baseline*puWeight*BTagWeight"; //2018
  const TString wgtvar18 = lumistr+"*1000*Stop0l_evtWeight*puWeight*BTagWeight";
// photon trigger eff.
...
// Tag-and-Probe Lepton SF
  ...
  const TString lepvetowgt18 =      wgtvar18;
// 1LCR Lepton SF
...
const TString trigDiLepCR = " && passtrigdilepOR && dileppt>200";
  const TString datasel = " && Pass_EventFilter  && Pass_HT && Pass_JetID && Pass_CaloMETRatio ";//&& Pass_dPhiMETMedDM ";//&& Pass_exHEMVeto30";
  //const TString datasel ="&& Pass_QCDCR_highDM && Pass_CaloMETRatio ";// && Pass_exHEMVeto30";//njet_j30 >=2
  const TString dataselHEM = "&& Pass_QCDCR_lowDM && Pass_CaloMETRatio && Pass_exHEMVeto30 && njet_j30 >=2";   
  const TString qcdSpikeRemovals = "";// && (!(lumi==40062 && event==91000735))";
  const TString dphi_invert = "&& Pass_dPhiQCD";// && (Jet_dPhiMET[0]<0.1 && Jet_dPhiMET[1]<0.1 && Jet_dPhiMET[2] < 0.1)";
...
BaseConfig qcdConfig(){
  ...
  if(is2016){ 
    ...
    config.addSample("qcd-cr",      "QCD",              "qcd_smear_2016",         qcdwgt,   datasel + trigSR + dphi_invert + qcdSpikeRemovals);
    config.addSample("qcd-withveto-cr",  "QCD",         "qcd_smear_2016",       qcdvetowgt,  datasel + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
  ...
  // qcdcr: other bkg subtraction
    ...
    // config.addSample("rare-cr",         "rare",         "rare_2016",            lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
    config.addSample("znunu-cr",       "Znunu",         "znunu_2016",           lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
  }else if(is2017){
    config.addSample("data-cr",     "Data",             "data_2017",  "1.0",      datasel + trigSR + vetoes + dphi_invert);
    config.addSample("qcd-cr",      "QCD",              "qcd_smear_2017_v2",         qcdwgt,   datasel + trigSR + dphi_invert + qcdSpikeRemovals);
    config.addSample("qcd-withveto-cr",  "QCD",         "qcd_smear_2017_v2",       qcdvetowgt,  datasel + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
    config.addSample("qcd-withveto-sr",  "QCD",         "qcd_smear_2017_v2",       qcdvetowgt,  datasel + trigSR + vetoes + qcdSpikeRemovals);
  // qcdcr: other bkg subtraction
    config.addSample("ttbar-cr",       "t#bar{t}",      "ttbar_2017",           lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
    config.addSample("wjets-cr",       "W+jets",        "wjets_2017",           lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
    config.addSample("minor-cr",       "minor",         "minor_2017",           lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
    //config.addSample("rare-cr",         "rare",         "rare_2017",            lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
    config.addSample("znunu-cr",       "Znunu",         "znunu_2017",           lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
}else if(is2018){
    if(PostHEM){
      config.addSample("data-cr",     "Data",             "data_2018D",  "1.0",      dataselHEM + trigSR + vetoes + dphi_invert);
      config.addSample("qcd-cr",      "QCD",              "qcd_orig_2018",         qcdwgt18,   dataselHEM + trigSR + dphi_invert + qcdSpikeRemovals);
      config.addSample("qcd-withveto-cr",  "QCD",         "qcd_orig_2018",       qcdvetowgt18,  dataselHEM + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
      config.addSample("qcd-withveto-sr",  "QCD",         "qcd_orig_2018",       qcdvetowgt18,  dataselHEM + trigSR + vetoes + qcdSpikeRemovals);
  // qcdcr: other bkg subtraction
      config.addSample("ttbar-cr",       "t#bar{t}",      "ttbar_2018",           lepvetowgt18,  dataselHEM + trigSR + vetoes + dphi_invert);
      config.addSample("wjets-cr",       "W+jets",        "wjets_2018",           lepvetowgt18,  dataselHEM + trigSR + vetoes + dphi_invert);
      config.addSample("minor-cr",       "minor",         "minor_2018",           lepvetowgt18,  dataselHEM + trigSR + vetoes + dphi_invert);
      // config.addSample("rare-cr",         "rare",         "rare_2018",            lepvetowgt18,  dataselHEM + trigSR + vetoes + dphi_invert);
      config.addSample("znunu-cr",       "Znunu",         "znunu_2018",           lepvetowgt18,  dataselHEM + trigSR + vetoes + dphi_invert);   
}else{
      config.addSample("data-cr",     "Data",             "data_2018N",  "1.0",      datasel + trigSR + vetoes + dphi_invert);
      config.addSample("qcd-cr",      "QCD",              "qcd_orig_2018",         qcdwgt18,   datasel + trigSR + dphi_invert + qcdSpikeRemovals);
      config.addSample("qcd-withveto-cr",  "QCD",         "qcd_orig_2018",       qcdvetowgt18,  datasel + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
      config.addSample("qcd-withveto-sr",  "QCD",         "qcd_orig_2018",       qcdvetowgt18,  datasel + trigSR + vetoes + qcdSpikeRemovals);
  // qcdcr: other bkg subtraction
      config.addSample("ttbar-cr",       "t#bar{t}",      "ttbar_2018",           lepvetowgt18,  datasel + trigSR + vetoes + dphi_invert);
      config.addSample("wjets-cr",       "W+jets",        "wjets_2018",           lepvetowgt18,  datasel + trigSR + vetoes + dphi_invert);
      config.addSample("minor-cr",       "minor",         "minor_2018",           lepvetowgt18,  datasel + trigSR + vetoes + dphi_invert);
      //  config.addSample("rare-cr",         "rare",         "rare_2018",            lepvetowgt18,  datasel + trigSR + vetoes + dphi_invert);
      config.addSample("znunu-cr",       "Znunu",         "znunu_2018",           lepvetowgt18,  datasel + trigSR + vetoes + dphi_invert);
    }
  }
