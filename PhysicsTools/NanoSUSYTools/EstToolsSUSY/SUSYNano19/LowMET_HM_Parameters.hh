...
namespace EstTools{
  ...
  const TString outputdir = "LowMET_HM_16";
  const TString datadir = "./";
  const TString lumistr = "21068";//"38630";//"21068";//13220.3//27987.8
const TString lumistr_2016 = "21068"; //Units are in pb
TString getLumi(){return lumistr(TRegexp("[0-9]+.[0-9]"));}

// lumi and base weight
  const TString wgtvar = lumistr_2016+"*Stop0l_evtWeight*Stop0l_trigger_eff_MET_loose_baseline_QCD*puWeight*BTagWeight*topTagWeight*restopTagWeight*wTagWeight*softbTagWeight*PrefireWeight"; //2016 PrefireWeight
  const TString wgtvarsr = lumistr_2016+"*Stop0l_evtWeight*Stop0l_trigger_eff_MET_loose_baseline*puWeight*BTagWeight*topTagWeight*restopTagWeight*wTagWeight*softbTagWeight*PrefireWeight";
// photon trigger eff.
...
const TString trigDiLepCR = " && passtrigdilepOR && dileppt>200";
  ...
  const TString datasel = " && Pass_EventFilter && Pass_HT && Pass_JetID && Pass_CaloMETRatio && (run < 319077 || run==1)";// && Pass_exHEMVeto30 ";
  //const TString datasel = " && Pass_EventFilter && Pass_HT30 && Pass_JetID && Pass_CaloMETRatio && Pass_exHEMVeto30";
...
BaseConfig lepConfig(){
  ...
  if (ADD_LEP_TO_MET){
    ...
    config.addSample("ttbar-2016",       "t#bar{t}",      inputdir+"ttbar",           onelepcrwgt+"*ISRWeight", datasel + trigLepCR + lepcrsel);
    config.addSample("wjets-2016",       "W+jets",        inputdir+"wjets",           onelepcrwgt, datasel + trigLepCR + lepcrsel);
...
//    config.addSample("qcd",         "QCD",           "lepcr/qcd",             onelepcrwgt, datasel + trigLepCR + lepcrsel);
  ...
  config.addSample("ttbar-2016-sr",       "t#bar{t}",      inputdir+"ttbar",                lepvetowgt+"*ISRWeight", datasel + vetoes);
  config.addSample("diboson-2018postHEM-sr", "Diboson",       inputdir+"diboson",           lepvetowgt, datasel + vetoes);
...
BaseConfig qcdConfig(){
  ...
  config.inputdir = inputdir;
  config.outputdir = outputdir+"/QCD_HM_16";
  config.header = "#sqrt{s} = 13 TeV, "+lumistr+" fb^{-1}";

  // qcdcr
  config.addSample("data-cr",     "Data",             "data_2016",          "1.0",      datasel + trigSR + vetoes + dphi_invert);
  config.addSample("qcd-cr",      "QCD",              "qcd_smear_sf_2016",       qcdwgt,      datasel + trigSR + dphi_invert + qcdSpikeRemovals);

  config.addSample("qcd-withveto-cr",  "QCD",         "qcd_smear_sf_2016",       qcdvetowgt,  datasel + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
  config.addSample("qcd-withveto-sr",  "QCD",         "qcd_smear_sf_2016",       qcdvetowgtsr,  datasel + trigSR + vetoes + qcdSpikeRemovals);
  ...
  config.addSample("qcd-norm",       "QCD",           "qcd_smear_sf_2016",       lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut + qcdSpikeRemovals);

  // qcdsr
  config.addSample("qcd-sr",         "QCD",   "qcd_smear_sf_2016",       qcdwgtsr,      datasel + trigSR + qcdSpikeRemovals);
  if (SPLITTF){
    config.addSample("qcd-sr-int",     "QCD", "qcd_smear_sf_2016",       qcdwgtsr,      datasel + trigSR + dphi_cut + qcdSpikeRemovals);
  }
