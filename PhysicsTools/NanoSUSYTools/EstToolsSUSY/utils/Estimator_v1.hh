...
//    drawHeader(header_);
  ...
  TH1* plotDataMC(const BinInfo& var_info, const vector<TH1D*> mc_samples,TH1D* data_sample,const Category& category , bool norm_to_data =false, TString norm_cut ="", bool plotlog =false ,std::function<void(TCanvas*)> *plotextra = nullptr){
    ...
    auto cut = config.sel + " && " + category.cut + TString(selection_=="" ? "" : " && "+selection_);
    prepHists({data_sample});
    if (saveHists_) saveHist(data_sample);
    addLegendEntry(leg, data_sample, "LP");//check it

    vector<TH1D*> mc ={"nonQCD","QCD"};
    for(auto &scomb: mc){
