...
const TString lumistr = "38.9";

TString getLumi(){return lumistr(TRegexp("[0-9]+.[0-9]"));}

// lumi and base weight
const TString wgtvar = lumistr+"*1000*Stop0l_evtWeight*BTagWeight*topTagWeight*wTagWeight*restopTagWeight*softbTagWeight";//*restopSF //2016
...
const TString datasel = " && Pass_EventFilter && Pass_HT && Pass_JetID && Pass_CaloMETRatio && (run < 319077 || (run >= 319077 && Pass_exHEMVeto20)) && restopTagWeight < 2 && restopTagWeight >0";
...
BaseConfig qcdConfig(){
  ...
  // qcdcr
  config.addSample("data-cr",     "Data",             "data_2018D",  "1.0",      sdataselHEM + trigSR + vetoes + dphi_invert);
  config.addSample("qcd-cr",      "QCD",              "qcd_smear_2018",         qcdwgt,   dataselHEM + trigSR + dphi_invert + qcdSpikeRemovals);

  config.addSample("qcd-withveto-cr",  "QCD",         "qcd_smear_2018",       qcdvetowgt,  dataselHEM + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
  config.addSample("qcd-withveto-sr",  "QCD",         "qcd_smear_2018",       qcdvetowgt,  dataselHEM + trigSR + vetoes + qcdSpikeRemovals);

  // qcdcr: other bkg subtraction                                                                                                                                                   
if(is2016){
    config.addSample("ttbar-cr",       "t#bar{t}",      "ttbar_2018",           lepvetowgt,  dataselHEM + trigSR + vetoes + dphi_invert);
  }else{
    config.addSample("ttbar-cr",       "t#bar{t}",    "ttbar_2018",           lepvetowgt,  dataselHEM + trigSR + vetoes + dphi_invert);
  }
  config.addSample("wjets-cr",       "W+jets",        "wjets_2018",           lepvetowgt,  dataselHEM + trigSR + vetoes + dphi_invert);
  config.addSample("minor-cr",       "minor",         "minor_2018",           lepvetowgt,  dataselHEM + trigSR + vetoes + dphi_invert);
  config.addSample("znunu-cr",       "Znunu",         "znunu_2018",           lepvetowgt,  dataselHEM + trigSR + vetoes + dphi_invert);

  // onelepcr: norm correction for other bkg subtraction                                                                                                                           
  config.addSample("data-norm",      "Data",          "data_2018D", "1.0",     sdataselHEM + trigSR + revert_vetoes + dphi_cut);
  if(is2016){
    config.addSample("ttbar-norm",     "t#bar{t}",      "ttbar_2018",           lepselwgt, dataselHEM + trigSR + revert_vetoes + dphi_cut);
  }else{
    config.addSample("ttbar-norm",   "t#bar{t}",      "ttbar_2018",           lepselwgt,   dataselHEM + trigSR + revert_vetoes + dphi_cut);
  }
  config.addSample("wjets-norm",     "W+jets",        "wjets_2018",           lepselwgt,   dataselHEM + trigSR + revert_vetoes + dphi_cut);
  config.addSample("minor-norm",     "minor",         "minor_2018",           lepselwgt,   dataselHEM + trigSR + revert_vetoes + dphi_cut);
  config.addSample("qcd-norm",       "QCD",           "qcd_smear_2018",       lepselwgt,   dataselHEM + trigSR + revert_vetoes + dphi_cut + qcdSpikeRemovals);
  // qcdsr                                                                                                                                                                       
  config.addSample("qcd-sr",         "QCD",           "qcd_smear_2018",        qcdwgt,      dataselHEM + trigSR + qcdSpikeRemovals);

  // samples for splitting the TF, optional (see z.splitTF)                                                                                                                       
  if (SPLITTF){
    config.addSample("qcd-sr-int",     "QCD",           "qcd_smear_2018",       qcdwgt,      dataselHEM + trigSR + dphi_cut + qcdSpikeRemovals);
  }
