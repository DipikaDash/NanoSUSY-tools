...
vector<Quantity> QCDPred(){
  ...
  z.printYieldsTableLatex({"_DATA", "_TF", "_pred"}, labelMap, "yields_qcd_lm_anaw.tex","lm", digits); //LM
  if(z.splitTF){
    z.printYieldsTableLatex({"_DATA", "_TF", "_QCDTF_CR_to_SR_noextrap", "_QCDTF_SR_extrap", "_pred"}, labelMap, "yields_qcd_hm_anaw.tex","hm", digits);
  }else{
    z.printYieldsTableLatex({"_DATA", "_TF", "_pred"}, labelMap, "yields_qcd_hm.tex_anaw","hm", digits);
  }
