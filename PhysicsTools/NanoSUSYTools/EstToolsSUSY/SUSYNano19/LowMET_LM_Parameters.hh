...
const TString outputdir = "LowMET_LM_17BE";

  const TString datadir = "./";

  const TString lumistr = "27987.8";//"27987.8";//"13220.3";
const TString lumistr_2016 = "27987.8"; //Units are in pb


TString getLumi(){return lumistr(TRegexp("[0-9]+.[0-9]"));}

// lumi and base weight
  const TString wgtvar = lumistr_2016+"*Stop0l_evtWeight*Stop0l_trigger_eff_MET_loose_baseline_QCD*17BtoEpuWeight*BTagWeight*topTagWeight*wTagWeight*restopTagWeight*softbTagWeight*PrefireWeight"; //2016 PrefireWeight
  const TString wgtvarsr = lumistr_2016+"*Stop0l_evtWeight*Stop0l_trigger_eff_MET_loose_baseline*17BtoEpuWeight*BTagWeight*topTagWeight*wTagWeight*restopTagWeight*softbTagWeight*PrefireWeight";  
...
const TString trigDiLepCR = " && passtrigdilepOR && dileppt>200";
  //const TString datasel = " && Pass_EventFilter && Pass_HT && Pass_JetID && Pass_CaloMETRatio && (run >= 319077||run==1) && Pass_exHEMVeto30";
  const TString datasel = " && Pass_EventFilter && Pass_HT && Pass_JetID && Pass_CaloMETRatio && (run < 319077 || run==1)";// && Pass_exHEMVeto30 ";
...
BaseConfig qcdConfig(){
  ...
  config.addSample("qcd-cr",      "QCD",              "qcd_smear_sf_2017",       qcdwgt,      datasel + trigSR + dphi_invert + qcdSpikeRemovals);

  config.addSample("qcd-withveto-cr",  "QCD",         "qcd_smear_sf_2017",       qcdvetowgt,  datasel + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
  config.addSample("qcd-withveto-sr",  "QCD",         "qcd_smear_sf_2017",       qcdvetowgtsr,  datasel + trigSR + vetoes + qcdSpikeRemovals);

  // qcdcr: other bkg subtraction
  //config.addSample("ttbar-cr",       "t#bar{t}",      "ttbar_2017",        lepvetowgt+"*ISRWeight",  datasel + trigSR + vetoes + dphi_invert);
  ...
  config.addSample("data-norm",      "Data",          "data_2017BtoE",          "1.0",       datasel + trigSR + revert_vetoes + dphi_cut);
  //config.addSample("ttbar-norm",     "t#bar{t}",      "ttbar_2017",        lepselwgt+"*ISRWeight",   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("ttbar-norm",     "t#bar{t}",      "ttbar_2017",        lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("wjets-norm",     "W+jets",        "wjets_2017",        lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("minor-norm",        "minor",      "minor_2017",        lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("qcd-norm",       "QCD",           "qcd_smear_sf_2017",       lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut + qcdSpikeRemovals);

  // qcdsr
  config.addSample("qcd-sr",         "QCD",           "qcd_smear_sf_2017",       qcdwgtsr,      datasel + trigSR + qcdSpikeRemovals);
  if (SPLITTF){
    config.addSample("qcd-sr-int",     "QCD",           "qcd_smear_sf_2017",       qcdwgtsr,      datasel + trigSR + dphi_cut + qcdSpikeRemovals);
  }
...
BaseConfig sigConfig(){
  ...
  config.inputdir = inputdir;
  config.outputdir = outputdir+"/sig30";
  config.header = "#sqrt{s} = 13 TeV, "+lumistr+" fb^{-1}";

  config.addSample("data-sr",        "Data",      "data_2017BtoE",                    "1.0",  datasel + trigSR + vetoes);
 
  // raw MC w/o top/W SF
  // config.addSample("znunu-raw-sr",                 "Z#rightarrow#nu#nu", "znunu_2016",           lepvetowgt_no_wtopsf,    	     datasel + vetoes);
  // config.addSample("ttbar-raw-sr",                 "t#bar{t}",           "ttbar_2016",           lepvetowgt_no_wtopsf+"*ISRWeight",    datasel + vetoes);
  // config.addSample("wjets-raw-sr",                 "W+jets",             "wjets_2016",           lepvetowgt_no_wtopsf,                 datasel + vetoes);
  // config.addSample("minor-raw-sr",                    "minor",            "minor_2016",              lepvetowgt_no_wtopsf,                 datasel + vetoes);
  // config.addSample("qcd-raw-sr",                   "QCD",                "qcd_smear_2016",             lepvetowgt_no_wtopsf,                 datasel + vetoes);
